        ...
        function center() {
            ...
            if ( _currentIndex >= 0 ) {
                ...
                if ( transformSupport ) {
                    ...
                    _container.css('-webkit-transform', 'translateX(' + _itemOffsets[_currentIndex] + 'px)');
                } else {
        ...
        function touchEvents(elem) {
            if ( settings.touch ) {
                ...
                elem.hammer({
                  ...
                  var g = e.gesture;
                  _startDrag = ( g.pointers ? g.pointers[0].clientX : e.deltaX );
                  _startDragY = ( g.pointers ? g.pointers[0].clientY : e.deltaY );
                          //e.preventDefault();
                }).bind('panmove', function(e) {
                  if ( _startDrag !== false ) {
                    var g = e.gesture;
                    x = ( g.pointers ? g.pointers[0].clientX : e.deltaX );
                    y = ( g.pointers ? g.pointers[0].clientY : e.deltaY );
                    offsetY = y - _startDragY;
                    offsetX = x - _startDrag;

                    if ( Math.abs(offsetY) < 100 && Math.abs(offsetX) >= 3 ) {
                        _touchJump((offsetX < 0 ? 'next' : 'prev'));
                        _startDrag = x;
                        e.preventDefault();
                    }
