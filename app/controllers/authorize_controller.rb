class AuthorizeController < ApplicationController
  ...
  def weixin_redirect_url
    ...
    if $client.is_valid?
      ...
      #   "privilege"=>[]
      # }
      profile = $client.get_oauth_userinfo('wx41b2029d52fb7900', access_token).result

      unless User.where("data @> ?", {weixin_openid: profile['openid']}.to_json).empty?
        User.create(
          weixin_openid: profile['openid'],
          username: profile['nickname'],
          sex: profile['sex'],
          image: profile['headimgurl'],
          languange: profile['language'],
          city: profile['city'],
